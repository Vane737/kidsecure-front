    ...
    <table className="table-fixed w-full font-sans text-left shadow-md px-9">
      <thead className="bg-gray-200 h-14 shadow-sm">
        <tr className="">
          ...
          <th className="px-5 font-semibold">PADRES</th>
          <th className="px-5 font-semibold">PERSONA AUTORIZADA</th>
        ...
        {body.map((b, i) => (
          <tr className="border-b border-gray-20 h-14 text-left" key={i}>
            {Object.keys(b).map((value, idx) => (
              value === 'authorizedPersons' || b[value] === '' ? null : (
                <td className="font-normal pl-5" key={idx}>
                  {value === 'birthdate' ? (
                    // Si la propiedad es 'birthdate', formatea la fecha
                    formatDate(b[value])
                  ) : (
                    // De lo contrario, muestra el valor sin formato
                    b[value]
                  )}
                </td>
              )
            ...
            <td>
              <button
                className="bg-primary rounded-md p-2 font-semibold pr-4 pl-4 text-white mr-3"
                onClick={() => handleClickCreate(b.id, false )}
              >
                <PlusIcon className="h-4 w-4" />{" "}
              </button>
